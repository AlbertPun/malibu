...
class ObjectManager {
  ...
  async getHeight(objectid: ObjectId, peer: Peer): Promise<number> {
    let height = 0;
    let currentBlock = Block.fromNetworkObject(
      <BlockObjectType>await this.retrieve(objectid, peer)
    );

    while (currentBlock.previd !== null) {
      height++;
      currentBlock = Block.fromNetworkObject(
        <BlockObjectType>await this.retrieve(currentBlock.previd, peer)
      );
    }
